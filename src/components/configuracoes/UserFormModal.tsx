...
import { X, Save, User as UserIcon, Mail, Shield, ShieldCheck, MapPin, Check, Building2, Loader2 } from "lucide-react";
import { cn } from "@/lib/utils";
import { getDelegations, getAppRoles } from "@/app/configuracoes/actions";
...
export default function UserFormModal({ initialData, onClose }: UserFormProps) {
  ...
  useEffect(() => {
    async function loadMetadata() {
      try {
        const [dels, roles] = await Promise.all([getDelegations(), getAppRoles()]);
        setDelegations(dels);
        setAppRoles(roles);
      } catch (err) {
  ...
  const standardRoles = appRoles.filter(r => r.scope === "Standard").map(r => r.name);
  const masterRoles = appRoles.filter(r => r.scope === "Master").map(r => r.name);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Check if at least one role is assigned unless they are globally Admin
    const hasRoles = formData.permissions.some((p: any) => p.roles.length > 0);
    if (!formData.isAdmin && !hasRoles) {
      alert("Selecione pelo menos uma delegação e um perfil.");
      return;
    }
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-[2.5rem] w-full max-w-4xl overflow-hidden border-2 border-zinc-100 dark:border-zinc-800 shadow-2xl flex flex-col max-h-[90vh]">
        ...
        {loading ? (
          <div className="flex-1 flex flex-col items-center justify-center space-y-4">
            ...
            <p className="text-[10px] font-black uppercase text-zinc-400 tracking-[0.2em]">A carregar dados...</p>
          </div>
        ) : (
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {delegations.map(del => {
                    ...
                    const availableRoles = del.is_master ? masterRoles : standardRoles;
                      ...
                      >
                        <div className="flex items-center gap-6 p-3">
                          ...
                          >
                            ...
                            {del.is_master && <span className="text-[8px] font-black bg-white/20 px-1.5 py-0.5 rounded ml-auto">MASTER</span>}
                          </button>
        ...
        <div className="px-8 py-6 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between bg-zinc-50/50 dark:bg-zinc-900/50 flex-shrink-0">
            ...
            <div className="flex gap-4">
              ...
              <button 
                ...
                className="flex items-center gap-3 px-10 py-3 bg-blue-600 text-white rounded-2xl font-black uppercase text-[11px] tracking-widest hover:bg-blue-700 transition-all shadow-xl shadow-blue-500/20 active:translate-y-1"
              >
